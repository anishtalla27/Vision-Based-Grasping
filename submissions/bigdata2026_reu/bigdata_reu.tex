% NSF/UMBC REU Symposium at IEEE BigData 2026, short paper (6 pages max). Build: tectonic bigdata_reu.tex
...
\title{Three Ways to Miss a Grasp: A Same-Metric Benchmark of Rule-Based, Learned, and Zero-Shot Vision-Language Grasp Prediction with Object-Clustered Inference}

\author{\IEEEauthorblockN{Anish Talla}
\IEEEauthorblockA{\textit{High School Student}, {[}SCHOOL NAME{]}\\
...
correction. GPT-4o passed on 12.4\% of calls across five independent runs
per image. Object-clustered resampling left the order unchanged. Their errors were different. The heuristic struggled with
...
size. The failure is in the grasp choice, not in emitting coordinates.
\end{abstract}
...
where each system goes wrong, and finally test the most natural explanation
...
Of the 884 frame boundaries, 251 (28.4\%) fell into the ambiguous band.
Comparing raw pixel differences between consecutive frames had failed,
because the dataset rotates each object between shots and same-object and
different-object scores overlapped; the mask descriptors separated them.
...
uncertain call and every high-risk ``different object'' call; 139
boundaries were decided by hand and 112 confident same-object calls were
accepted unreviewed, since an error in that direction only merges groups.
Six boundaries stayed uncertain after review; one image from the smaller
side of two of them was dropped, which is the 885 to 883 difference.
Published descriptions of Cornell report around 240 objects, and the
shortfall to 234 is the expected effect of thresholds biased toward
merging. Table
...
System C sends the native $640\times480$ RGB image to GPT-4o
(\texttt{openai/gpt-4o}) through OpenRouter \cite{openai2024}, with a
400-token cap, temperature left at the provider default, and no seed
pinned, since run-to-run variation was one of the quantities measured. It uses no grasp-specific training. The prompt asks for two
fingertip coordinates and a jaw width in JSON. Those three values fix all
four rectangle corners, which then pass through the same function that
produced every ground-truth angle, so no angle convention had to be matched
by hand and the model was never asked for an angle. I developed the prompt on 30 training images and froze it before
test.
...
marks at three positions; four marks at the centroid only, drawn at real
size; and the same four marks drawn at one uniform length, with the prompt
stating that length is not to scale. The last two exist because on thin
...
\subsection{Orientation Stratification}

Each system's design predicts something about orientation. System A cannot
express a diagonal grasp, System B's encoding handles rotation by
construction, and System C has no structural position. I therefore split
the test images by whether any labeled grasp lies within $15^\circ$ of
$0^\circ$ or $90^\circ$ (axis-aligned, 103 images) or none does
(diagonal, 20 images). Only System A's drop on the diagonal stratum was
predicted before scoring; the other two directions are exploratory.
...
points [14.8, 38.3], $p=0.0002$; its 4.3-point lead over round 1
[$-5.8$, $+13.7$] does not exclude zero, so the second round is best read
as the same result with its uncertainty measured. Table~\ref{tab:round2}
lists every configuration that touched the test split. Averaged final
weights beat the validation-selected checkpoint of their own run on five
of six second-round runs, and test-time augmentation added between 0.0 and
5.7 points, about three on average. The first-round recipe's final weights
at two new seeds scored 80.5\% and 62.6\%, which is the instability the
second recipe removed.

\begin{table}[t]
\caption{System B, both rounds, same 123 test images. Round 2 reports the
mean over three seeds of the final-epoch averaged weights; no checkpoint
was selected.}
\label{tab:round2}
\centering
\footnotesize
\begin{tabular}{llr}
\toprule
Round & Configuration & Test \\
\midrule
1 & ResNet18, seed 42, best val.\ checkpoint & 79.7\% \\
1 & ResNet34, seed 42, best val.\ checkpoint & 70.7\% \\
1 & custom 1.0M-param CNN, seed 42 & 23.6\% \\
1 recipe & ResNet18, seeds 0 and 1, best val.\ checkpoint & 82.9, 80.5\% \\
2 & ResNet18, 3 seeds, final weights, TTA & 82.6\% \\
2 & ResNet34, 3 seeds, final weights, TTA & \textbf{84.0\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\columnwidth]{compare_0285.png}
...
agreement was 22.0\%, mean pairwise angle spread $5.8^\circ$, and mean
pairwise IoU 0.13. A consensus rule that keeps whichever repeat agrees most
with the other four scored 12.2\%, no better than one call. Restricting
to the 21.1\% of images where at least 40\% of repeat pairs agreed lifts
best-of-five to 57.7\% on those images, so self-agreement carries signal
about which calls to trust, though it is not a fourth accuracy figure. All five calls failed on
...
did not. Model contamination by the public dataset was checked weakly, by
asking the model to identify the source of ten test images; it recognized
none, and contamination could only have raised the lowest score.

On the orientation strata, System A scored 61.2\% on the 103 axis-aligned
images and 40.0\% on the 20 diagonal ones, System B 77.7\% and 90.0\%,
and System C 12.0\% and 14.0\%. The three systems move in three
directions, but the diagonal intervals are wide, and only A's drop was
predicted beforehand. A second split by the number of labeled grasps per
image behaved opposite to a more-labels-means-easier reading: all three
systems did worst on the images with 8 to 25 labels (48.0, 64.0, and
11.2\%), which suggests the count tracks object complexity.

Systems A and B agreed on 74 of 123 images (both correct on 60, both wrong
on 14); of the 49 disagreements, B alone was correct on 38 and A alone on
11, significant under McNemar's exact test ($p=1.4\times10^{-4}$).

\subsection{System D}

Table~\ref{tab:menus} gives the three menus, and Fig.~\ref{fig:som}
shows one marked image. On none of them does the
...
correct. Every reply parsed; the 55 calls on the eleven images with no
segmentation mask, and so no candidates, count as misses for System D and
for every row it is compared with. Self-agreement across the five repeats,
measured on the physical candidate with numbering re-shuffled each call,
was 44.3\%, twice System C's, and best-of-five was 39.0\%. On the full
menu the end-pinch candidate was chosen on 342 of 560 calls and the
short-axis centroid candidate, which passes on 60.7\% of images, on 34.
The sparse menu's 7.8-point lead over the full menu excludes zero, but its
floor is 4.5 points higher, so the gain belongs to the menu, not the model. The candidate at the centroid closing across
the short axis, with no model at all, scored 55.3\% [41.8, 68.0], level
with System A.

\begin{figure}[t]
\centering
\includegraphics[width=0.5\columnwidth]{som_0129.png}
\caption{The marked image for one test object as System D saw it on the
full menu. The model chose mark 6, the end pinch along the bean, on all
five repeats, describing it as gripping ``the central narrow part of the
...
reached 12.4\% per call. Their error locations add context to that
...
results. Anthropic Claude Code helped with
code development, made the first-pass calls on ambiguous object
boundaries, and implemented the second training round and System D under
my design. I designed the study, made the research decisions, ran and checked
