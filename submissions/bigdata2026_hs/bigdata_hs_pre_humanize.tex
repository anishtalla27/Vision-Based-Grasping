...
% IEEE BigData 2026 High School Symposium. Build: tectonic bigdata_hs.tex

\title{Where Multimodal Grasp Prediction Fails: A Same-Metric Benchmark of Rule-Based, Learned, and Zero-Shot Vision-Language Systems with Repeated-Measures Evaluation}
...
\begin{abstract}
Benchmarks of multimodal models often change the input, the output, or
the scoring rule when the method changes, which makes their accuracies
hard to compare. I compared three ways to predict a robotic grasp from an
...
resampling, which widened every interval and left the order unchanged. Their errors were different. The heuristic struggled with
...
\begin{IEEEkeywords}
multimodal data mining, vision-language models, benchmark evaluation,
repeated measures, clustered inference, robotic grasping, spatial grounding
\end{IEEEkeywords}
...
with the method. This paper treats that comparison as a data-analysis
problem: hold the data, the output format, and the scorer fixed, sample the
stochastic system enough times to measure its own variance, and draw
inferences at the level of the unit that is actually independent.
...
The VLM was queried 615 times, five single-turn calls per image, so that
run-to-run variation on identical input is measured rather than hidden
behind one draw. I first ask which system predicts the most valid
